...
4. Challenges to attend online opportunities (`other_challenge_opport`)
...
For each, the respondent either selected that STEM field  or selected other but their response is categorized as that field.

For example, for Agriculture & Natural Resources:
...
For each, the respondent either selected that challenge to online activities or selected other but their response is categorized as that challenge.
...
```{r 4.03, echo = FALSE, fig.height = 7, fig.width = 10}
p.gender_summary1
p.gender_summary2
```

```{r 4.04, echo = FALSE, fig.height = 7, fig.width = 10}
p.gender_summary3
```
...
f.plot.res_act <- function(dataset, activity, activity_name){
  ...
  d.res_act <- dataset %>% select(gender, all_of(activity)) %>% drop_na(gender) %>% filter(gender != "other_notsay")
...
```{r 5.04, fig.height=6, fig.width=8, echo=FALSE}
