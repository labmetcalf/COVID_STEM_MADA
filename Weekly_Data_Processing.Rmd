...
```{r 1.01}
k.data <- read_xlsx(here("DATA_04_Impacts_of_Covid-19_on_Malagasy_STEM_-_latest_version_-_False_-_2021-04-11-05-57-47.xlsx"))
```
...
df.stress.all <- df.stress %>% group_by(gender) %>% summarize(
  ...
  p.ss.anxiety    = length(which(stress_anxiety %in% 
                                c("modstress", "strongstress", "extrastress")))/length(stress_anxiety)) %>%
  melt(id.vars = "gender")
