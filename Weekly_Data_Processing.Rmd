...
df.stress.all <- df.stress.all %>% mutate(label = case_when(
  variable == "p.ss.delaywork" ~ "",
  variable == "p.ss.career" ~ "",
  variable == "" ~ "",
  variable == "" ~ "",
  variable == "" ~ "",
  variable == "" ~ "",
...
ggplot(data = df.stress.all, aes(x = gender, y = value, group = variable)) +
  ...
  geom_text_repel(data = df.stress.all %>% filter(gender == "female"), 
                  aes(label = paste0(variable, " : ", round(value*100, 2), "%")) , 
                  hjust = "left", 
                  fontface = "bold", 
                  size = 4, 
                  nudge_x = -.45, 
  ...
  labs(
    title = "Stress",
    subtitle = "n = 770",
    caption = "comparing women and men"
  )
...
### Section 6 | Preliminary data visualizations: COVID relevant highlights
...
df1 <- k.data %>% rename(index = "_index") %>% 
