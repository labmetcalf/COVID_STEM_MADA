...
**(3)** Renaming problematic column names

- `_index` to `index`
- `_submission_time` to `submission_time`

```{r 2.01.3, echo = FALSE}
k.data <- k.data %>% rename(index = `_index`) %>% rename(submission_time = `_submission_time`)


```

**(4)** Removing individuals with unlikely/illogical responses

- `index` = `990`: Individual sampled 2021-05-06 reported age of 100 years but walks to work each day

```{r 2.01.4, echo = FALSE}
k.data <- k.data %>% filter(!(today == "2021-05-06" & age == "1921" & index == 990))
```


***

#### Section 2.2 | Cleaning up "other" responses
...
print(unique(k.data$other_acad))
