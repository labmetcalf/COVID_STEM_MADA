...
k.data <- k.data %>%
  ...
  mutate(inst_priv = case_when(
    `inst/priv_inst` == "1"         ~ 1,
     ...
     TRUE ~ 0)) %>% 
  #inst_none
  group_by(`_index`) %>% mutate(inst_none = case_when(
     sum(inst_uni, inst_ngo, inst_gov, inst_lab, inst_priv, na.rm = TRUE) < 1 ~ 1,
     is.na(inst) ~ NA_real_,
     TRUE ~ 0)) %>% ungroup() %>%
  #Removing old institution type columns
  select(start:affiliation, inst_uni, inst_ngo, inst_gov, inst_lab, inst_priv, inst_none, office_build:`_index`)
```
...
k.data <- k.data %>%
  ...
  mutate(uninterested = case_when(
    `challenge_opport/anxiety_notinteres` == "1"                  ~ 1,
     ...
     TRUE ~ 0)) %>% 
  #chall_none
  group_by(`_index`) %>% mutate(chall_none = case_when(
